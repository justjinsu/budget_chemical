...
if __name__ == "__main__":
    ...
    file_paths = ['data/IEA_EDGAR_CO2_1970_2022.xlsx']
    ...
    approach_values = ['NY.GDP.MKTP.PP.KD'] # GDP
    period_values = [2022]
    ...
    filtered_budget_df = utils.filter_budget_df(normal_allocation, countries, temp_values, probability_values, approach_values, period_values)
    ...
    Ipathway = pathwayCalculation.pathwayCalculator(start_year, mid_year, end_year, current_co2['value'].values[0], filtered_budget_df['allocation'].values[0], 0.25)

    lineartozeropathway_df = Ipathway.linear_to_zero()
    lineartopathway_df = Ipathway.linear_pathway()
    splinepathway_df = Ipathway.spline_pathway()
    fixed_reduction_df = Ipathway.fixed_reduction_pathway()
